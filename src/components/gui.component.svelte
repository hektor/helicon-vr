<script>
  ...
  const initGui = () => {
    ...
    pane.addInput($settings, 'fov', { min: 15, max: 90, step: 1 }).on('change', ({ value }) => {
      setFov(value)
      updateCamera('fov')
    })
    pane.addInput($settings, 'near', { min: 1, max: 1000, step: 1 }).on('change', ({ value }) => {
      setNear(value)
      updateCamera('near')
    })
    pane.addInput($settings, 'far', { min: 1, max: 1000, step: 1 }).on('change', ({ value }) => {
      setFar(value)
      updateCamera('far')
    })
